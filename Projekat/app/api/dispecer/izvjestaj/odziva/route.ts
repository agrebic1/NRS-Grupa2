...
export async function GET(request: Request) {
  try {
    ...
    const { data: zahtjevi, error: zErr } = await db
      ...
      .select(
        'id, serviser_dodijeljen_id, final_priority, created_at, updated_at',
      )
      .eq('status', 'zavrseno')
    ...
    if (!zahtjevi || zahtjevi.length === 0) {
      return NextResponse.json({
        period: {
          od: odDat.toISOString().substring(0, 10),
          do: doDat.toISOString().substring(0, 10),
        },
        serviseri: [],
        ukupno: {
          broj_intervencija: 0,
          avg_odziv_minuta: null,
          avg_trajanje_minuta: null,
          sla_compliance_posto: null,
        },
    ...
    // Profili servisera
    const serviserIds = [
      ...new Set(zahtjevi.map((z: any) => z.serviser_dodijeljen_id as string)),
    ];
    ...
    for (const e of evidencije ?? []) {
      ...
      evidByZah.set(
        e.zahtjev_id,
        (evidByZah.get(e.zahtjev_id) ?? 0) + e.trajanje_minuta,
      );
    }

    // Grupiraj aktivnosti po zahtjev_id
    const aktByZah = new Map<
      number,
      { dodjelaAt: string | null; prihvatAt: string | null }
    >();
    for (const a of aktivnosti ?? []) {
      const cur = aktByZah.get(a.zahtjev_id) ?? {
        dodjelaAt: null,
        prihvatAt: null,
      };
    ...
    for (const z of zahtjevi) {
      ...
      if (akt?.dodjelaAt && akt?.prihvatAt) {
        const odziv =
          (new Date(akt.prihvatAt).getTime() -
            new Date(akt.dodjelaAt).getTime()) /
          60_000;
        if (odziv >= 0) {
          odzivMin = Math.round(odziv);
          agg.odzvMin.push(odzivMin);
        }
      }
      ...
      if (z.final_priority && z.final_priority in SLA_ROKOVI_SATI) {
        const slaMs =
          SLA_ROKOVI_SATI[z.final_priority as keyof typeof SLA_ROKOVI_SATI] *
          3_600_000;
        const trajMs =
          new Date(z.updated_at).getTime() - new Date(z.created_at).getTime();
        slaOk = trajMs <= slaMs;
        if (slaOk) agg.slaOk++;
      }
    ...
    for (const agg of aggMap.values()) {
      agg.intervencije.sort(
        (a, b) =>
          new Date(b.zavrseno_at).getTime() - new Date(a.zavrseno_at).getTime(),
      );
    }
    ...
    const serviseri: ServiserOdzivaRed[] = [...aggMap.entries()]
      .map(([sid, agg]) => {
        const profil = profilMap.get(sid);
        const slaPosto =
          agg.broj > 0 ? Math.round((agg.slaOk / agg.broj) * 100) : null;
        return {
          serviser_id: sid,
    ...
    const sviOdzivi = serviseri.flatMap((s) =>
      s.avg_odziv_minuta != null ? [s.avg_odziv_minuta] : [],
    );
    const sviTrajanje = serviseri.flatMap((s) =>
      s.avg_trajanje_minuta != null ? [s.avg_trajanje_minuta] : [],
    );
    const sviSla = serviseri.flatMap((s) =>
      s.sla_compliance_posto != null ? [s.sla_compliance_posto] : [],
    );
    ...
    const odgovor: IzvjestajOdzivaOdgovor = {
      ...
      ukupno: {
        ...
        sla_compliance_posto: sviSla.length
          ? Math.round(sviSla.reduce((s, v) => s + v, 0) / sviSla.length)
          : null,
      },
    };
