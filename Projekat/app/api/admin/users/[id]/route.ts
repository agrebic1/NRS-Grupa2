...
function porukaValidacije(err: z.ZodError): string {
  ...
  if (polje === 'broj_telefona')
    return 'Broj telefona je predugačak (maks. 30 znakova).';
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'SUPABASE_SERVICE_ROLE_KEY nije konfigurisan.' },
        { status: 503 },
      );
    }
    ...
    const { data: authData, error: authErr } =
      await admin.auth.admin.getUserById(ciljId);
    ...
    const [
      ...
      adb
        ...
        .select(
          'id_korisnika_usluge, is_premium, premium_status, premium_expires_at',
        )
      ...
      adb
        .from('osoba')
        .select(
          'ime, prezime, email, broj_telefona, adresa, bazna_latitude, bazna_longitude',
        )
        .eq('id_osobe', ciljId)
        .maybeSingle(),
    ]);
    ...
    const tip: 'korisnik' | 'uposlenik' = profKorisnik
      ? 'korisnik'
      : 'uposlenik';
...
export async function PATCH(
  ...
  try {
    ...
    if (rawObj && typeof rawObj === 'object' && !Array.isArray(rawObj)) {
      ...
      if ('bazna_latitude' in o)
        o.bazna_latitude = normalizujKoordinatu(o.bazna_latitude);
      if ('bazna_longitude' in o)
        o.bazna_longitude = normalizujKoordinatu(o.bazna_longitude);
    }
    const parsed = patchSchema.safeParse(rawObj);
    if (!parsed.success) {
      return NextResponse.json(
        { error: porukaValidacije(parsed.error) },
        { status: 400 },
      );
    }
    ...
    if (body.action === 'uredi_podatke') {
      const { data: authCilj, error: authCiljErr } =
        await admin.auth.admin.getUserById(ciljId);
      if (authCiljErr || !authCilj.user) {
        return NextResponse.json(
          { error: 'Korisnik nije pronađen u autentifikaciji.' },
          { status: 404 },
        );
      }
      ...
      if (!sacuvano?.id_osobe) {
        return NextResponse.json(
          {
            error:
              'Profil korisnika nije sačuvan. Provjerite da korisnik postoji u bazi.',
          },
          { status: 500 },
        );
      }
      ...
      return NextResponse.json({
        ...
        ...(auditGreska
          ? { upozorenje: `Audit zapis nije upisan: ${auditGreska}` }
          : {}),
      });
    }
