...
// `u_obradi` je samo prikazni (dashboard) status i NE postoji u koloni `status`.
const AKTIVNI_STATUSI = [
  'pending_review',
  'in_review',
  'potvrdeno',
  'dodijeljeno',
...
async function obradiSlaEskalacije(request: Request) {
  try {
    ...
    const antiSpamOd = new Date(
      sada.getTime() - ANTI_SPAM_SATI * 3_600_000,
    ).toISOString();
    ...
    const notifikacije = zaEskalaciju.flatMap((z) => {
      ...
      const kasnjenjeMins = Math.floor(
        (sada.getTime() - new Date(z.created_at).getTime()) / 60_000 -
          rokSati * 60,
      );
      const kasnjenjeStr =
        kasnjenjeMins >= 60
