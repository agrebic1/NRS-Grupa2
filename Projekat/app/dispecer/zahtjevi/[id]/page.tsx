...
import { ArrowLeft, CheckCircle2, ChevronRight, Lock, AlertTriangle, X, Star, MessageSquare, Ban, XCircle, Calendar, FileText } from 'lucide-react';
...
function SazetakZavrseneDispecer({ zahtjev, evidencije }: { zahtjev: any; evidencije: unknown[] }) {
  const status: string = zahtjev.status;

  if (status === 'zavrseno') {
    return (
      <div className="mb-6 rounded-2xl overflow-hidden"
        style={{ border: '1px solid rgba(34,197,94,0.3)', backgroundColor: 'rgb(255 255 255/0.95)' }}>
        <div className="flex items-center gap-3 px-5 py-4"
          style={{ backgroundColor: 'rgba(34,197,94,0.07)', borderBottom: '1px solid rgba(34,197,94,0.15)' }}>
          <CheckCircle2 className="h-5 w-5 flex-shrink-0" style={{ color: '#16A34A' }} />
          <div>
            <p className="font-bold" style={{ color: '#166534' }}>Serviser završio intervenciju</p>
            <p className="text-xs" style={{ color: '#15803D' }}>Čeka formalno zatvaranje dispečera</p>
          </div>
        </div>
        <div className="px-5 py-4 flex flex-col gap-2">
          {evidencije.length > 0 ? (
            <p className="text-sm" style={{ color: 'var(--first-octonary)' }}>
              Evidentirano {evidencije.length} {evidencije.length === 1 ? 'zapis' : evidencije.length < 5 ? 'zapisa' : 'zapisa'} rada.
              Pregledajte evidenciju ispod i formalno zatvorite intervenciju.
            </p>
          ) : (
            <div className="flex items-start gap-2 rounded-xl p-3"
              ...
              <AlertTriangle className="h-4 w-4 flex-shrink-0 mt-0.5" style={{ color: '#DC2626' }} />
              <p className="text-sm" style={{ color: '#DC2626' }}>
                Serviser nije evidentirao obavljeni rad. Zatvaranje nije moguće dok evidencija nije dodata.
              </p>
            </div>
          )}
        </div>
  ...
  if (status === 'zatvoreno') {
    return (
      <div className="mb-6 rounded-2xl overflow-hidden"
        style={{ border: '1px solid rgb(var(--first-quaternary-rgb)/0.35)', backgroundColor: 'rgb(255 255 255/0.95)' }}>
        <div className="flex items-center gap-3 px-5 py-4"
          style={{ backgroundColor: 'rgb(var(--first-quinary-rgb)/0.4)', borderBottom: '1px solid rgb(var(--first-quaternary-rgb)/0.2)' }}>
          <Lock className="h-5 w-5 flex-shrink-0" style={{ color: 'var(--first-secondary)' }} />
          <div>
            <p className="font-bold" style={{ color: 'var(--first-octonary)' }}>Intervencija formalno zatvorena</p>
            {zahtjev.closed_at && (
              <p className="text-xs" style={{ color: 'var(--first-nonary)' }}>
                {new Date(zahtjev.closed_at).toLocaleDateString('bs-BA', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            )}
          </div>
        </div>
        {zahtjev.closure_note && (
          <div className="px-5 py-4">
            <div className="flex items-start gap-2 mb-1.5">
              <FileText className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" style={{ color: 'var(--first-nonary)' }} />
              <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: 'var(--first-nonary)' }}>Napomena pri zatvaranju</p>
            </div>
            <p className="text-sm leading-relaxed rounded-xl border-l-4 py-2 pl-3 pr-4"
              style={{ borderLeftColor: 'var(--first-secondary)', backgroundColor: 'rgb(var(--first-quinary-rgb)/0.3)', color: 'var(--first-octonary)' }}>
              {zahtjev.closure_note}
            </p>
          </div>
        )}
  ...
  if (status === 'otkazano') {
    return (
      <div className="mb-6 rounded-2xl overflow-hidden"
        style={{ border: '1px solid rgba(100,116,139,0.3)', backgroundColor: 'rgb(255 255 255/0.95)' }}>
        <div className="flex items-center gap-3 px-5 py-4"
          style={{ backgroundColor: 'rgba(100,116,139,0.06)', borderBottom: '1px solid rgba(100,116,139,0.15)' }}>
          <Ban className="h-5 w-5 flex-shrink-0" style={{ color: '#64748B' }} />
          <div>
            <p className="font-bold" style={{ color: '#334155' }}>Zahtjev otkazan</p>
            <p className="text-xs" style={{ color: '#64748B' }}>
              {zahtjev.updated_at
                ? new Date(zahtjev.updated_at).toLocaleDateString('bs-BA', { day: 'numeric', month: 'long', year: 'numeric' })
        ...
        {zahtjev.cancel_reason && (
          <div className="px-5 py-4">
            <p className="text-sm leading-relaxed" style={{ color: 'var(--first-octonary)' }}>
              <span className="font-semibold">Razlog: </span>{zahtjev.cancel_reason}
            </p>
          </div>
        )}
      </div>
  ...
  if (status === 'odbijeno') {
    return (
      <div className="mb-6 rounded-2xl overflow-hidden"
        style={{ border: '1px solid rgba(220,38,38,0.25)', backgroundColor: 'rgb(255 255 255/0.95)' }}>
        <div className="flex items-center gap-3 px-5 py-4"
          style={{ backgroundColor: 'rgba(220,38,38,0.05)', borderBottom: '1px solid rgba(220,38,38,0.12)' }}>
          <XCircle className="h-5 w-5 flex-shrink-0" style={{ color: '#DC2626' }} />
          <div>
            <p className="font-bold" style={{ color: '#991B1B' }}>Zahtjev odbijen</p>
        ...
        {zahtjev.dispecer_napomene && (
          <div className="px-5 py-4">
            <p className="text-sm leading-relaxed" style={{ color: 'var(--first-octonary)' }}>
              <span className="font-semibold">Napomena: </span>{zahtjev.dispecer_napomene}
            </p>
          </div>
        )}
      </div>
...
function OcjenaDispecerPregled({ zahtjevId }: { zahtjevId: number }) {
  const [ocjena,   setOcjena]   = useState<OcjenaPodaci | null | undefined>(undefined);
  ...
  return (
    <div
      ...
      style={{ border: '1px solid rgb(var(--first-quaternary-rgb)/0.28)', backgroundColor: 'rgb(255 255 255/0.9)' }}
    >
      <div
        className="flex items-center gap-2 px-4 py-3"
        style={{ borderBottom: '1px solid rgb(var(--first-quaternary-rgb)/0.2)', background: 'linear-gradient(135deg, rgba(234,179,8,0.07) 0%, transparent 100%)' }}
      >
        <Star className="h-3.5 w-3.5" style={{ color: '#CA8A04' }} />
        <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: '#92400E' }}>
          Ocjena korisnika
        </p>
      </div>

      {ocjena === null ? (
        <p className="px-4 py-4 text-sm" style={{ color: 'var(--first-nonary)' }}>
            ...
            <span className="inline-flex gap-1">
              {[1, 2, 3, 4, 5].map((n) => (
                <Star
                  ...
                  style={{ color: n <= ocjena.ocjena ? '#FBBF24' : 'rgb(var(--first-quaternary-rgb)/0.4)' }}
                />
              ))}
            </span>
            <span className="text-sm font-bold tabular-nums" style={{ color: 'var(--first-octonary)' }}>
              {ocjena.ocjena}/5
            </span>
            <span className="text-xs" style={{ color: 'var(--first-nonary)' }}>
              {new Date(ocjena.created_at).toLocaleDateString('bs-BA', { day: 'numeric', month: 'long', year: 'numeric' })}
            </span>
          </div>
          {ocjena.komentar && (
            <div
            ...
            >
              <MessageSquare className="mt-0.5 h-4 w-4 flex-shrink-0" style={{ color: '#CA8A04' }} />
              <p className="text-sm italic leading-relaxed" style={{ color: 'var(--first-octonary)' }}>
  ...
  return (
    <AppShell uloga="dispecer" imeKorisnika="Dispečer">
      <div className="mx-auto max-w-6xl px-4 sm:px-0">
        ...
        {zahtjev && (() => {
          const terminalni = new Set(['zavrseno', 'zatvoreno', 'otkazano', 'odbijeno']);
          ...
          return (
            <>
              ...
              {jeTerminalni ? (
                <SazetakZavrseneDispecer zahtjev={zahtjev} evidencije={evidencije} />
              ) : (
