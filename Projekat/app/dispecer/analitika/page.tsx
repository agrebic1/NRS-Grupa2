...
import {
  ...
  DonutChart,
  LineChart,
} from '@/components/dispecer/grafovi/Grafovi';
...
export default function AnalitikaPage() {
  ...
  const ucitaj = useCallback(async () => {
    ...
    setGreska(null);
    try {
      const r = await fetch(`/api/dispecer/analitika?od=${od}&do=${doDat}`, {
        cache: 'no-store',
      });
  ...
  const slaPosto =
    slaUkupno > 0
      ? Math.round((podaci!.sla.na_vrijeme / slaUkupno) * 100)
      : null;
      ...
      {!ucitava && !greska && !imaPodataka && (
        ...
        >
          ...
          <p
            ...
            style={{ color: 'var(--first-nonary)' }}
          >
            Promijenite datumski opseg ili provjerite da li u sistemu postoje
            zahtjevi.
          </p>
        </div>
      )}
      ...
      {!ucitava && imaPodataka && podaci && (
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            <KpiKartica
              oznaka="Ukupno zahtjeva"
              vrijednost={String(podaci.ukupno_zahtjeva)}
              boja="var(--first-secondary)"
              Ikona={Inbox}
            />
            <KpiKartica
              oznaka="Završenih"
              vrijednost={String(podaci.ukupno_zavrsenih)}
              boja="#166534"
              Ikona={CheckCircle2}
            />
            <KpiKartica
              oznaka="Prosjek odziva"
              vrijednost={formatMin(podaci.avg_odziv_minuta)}
              boja="var(--first-primary)"
              Ikona={Clock}
            />
            <KpiKartica
              oznaka="Prosjek trajanja"
              vrijednost={formatMin(podaci.avg_trajanje_minuta)}
              boja="#D97706"
              Ikona={TrendingUp}
            />
            <KpiKartica
              ...
              Ikona={
                slaPosto != null && slaPosto < 70 ? AlertTriangle : CheckCircle2
              }
            />
          </div>
            ...
            <GrafKartica naslov="SLA usklađenost" Ikona={PieChart}>
              <DonutChart
                ...
                segmenti={[
                  {
                    labela: 'Na vrijeme',
                    vrijednost: podaci.sla.na_vrijeme,
                    boja: '#16A34A',
                  },
                  {
                    labela: 'Prekoračeno',
                    vrijednost: podaci.sla.prekoraceno,
                    boja: '#DC2626',
                  },
                  {
                    labela: 'Bez podataka',
                    vrijednost: podaci.sla.bez_podataka,
                    boja: 'rgb(var(--first-quaternary-rgb))',
                  },
                ]}
              />
            </GrafKartica>
          ...
          >
            ...
            <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <KpiKartica
                ...
                vrijednost={
                  podaci.ocjene.prosjecna_ocjena != null
                    ? `${podaci.ocjene.prosjecna_ocjena}/5`
                    : '-'
                }
                boja="#CA8A04"
                Ikona={Star}
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <GrafKartica naslov="Opterećenje po serviseru" Ikona={Users}>
              ...
              >
                ...
                <Link
                  href="/dispecer/izvjestaj/odziva"
                  className="font-semibold underline-offset-2 hover:underline"
                  style={{ color: 'var(--first-secondary)' }}
                >
                  izvještaj odziva
            ...
            <GrafKartica
              naslov="Trend završenih intervencija"
              Ikona={TrendingUp}
            >
              <LineChart
                tacke={podaci.trend_zavrsenih.map((t) => ({
                  labela: fmtKratkiDatum(t.datum),
                  vrijednost: t.broj,
                }))}
              />
