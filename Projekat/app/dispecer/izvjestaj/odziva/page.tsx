...
import type { IzvjestajOdzivaOdgovor, ServiserOdzivaRed } from '@/lib/servisirane/izvjestajiOdziva';
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: 'var(--first-nonary)' }}>
              Analiza performansi tima po vremenskom periodu
            </p>
          </div>
          {/* Filter datuma */}
        ...
        >
          ...
          {!ucitava && podaci && (
            <span className="text-xs" style={{ color: 'var(--first-nonary)' }}>
              {podaci.serviseri.length} serviser{podaci.serviseri.length === 1 ? '' : podaci.serviseri.length < 5 ? 'a' : 'a'}
            </span>
          )}
        </div>
              ...
              <tbody>
                {podaci.serviseri.map((s: ServiserOdzivaRed, i) => (
                  <tr
                    key={s.serviser_id}
                    style={{
                      borderBottom: i < podaci.serviseri.length - 1
                        ? '1px solid rgb(var(--first-quaternary-rgb)/0.18)'
                        : 'none',
                      backgroundColor: i % 2 === 0 ? 'transparent' : 'rgb(var(--first-quinary-rgb)/0.08)',
                    }}
                  >
                    ...
                    <td className="px-5 py-3.5">
                      {s.sla_compliance_posto != null ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="tabular-nums text-xs font-bold" style={{ color: slaBoja(s.sla_compliance_posto) }}>
