...
export function statusBoja(s: string): string {
  switch (s) {
    case 'pending_review':
    case 'na_cekanju':   return 'var(--first-nonary)';
    case 'in_review':    return '#7E22CE';
    case 'potvrdeno':    return '#0369A1';
    case 'dodijeljeno':  return 'var(--first-senary)';
    case 'u_radu':
    case 'u_izvrsenju':  return 'var(--first-secondary)';
    case 'zavrseno':     return '#166534';
    case 'zatvoreno':    return 'var(--first-nonary)';
    case 'otkazano':     return '#DC2626';
    case 'odbijeno':     return '#991B1B';
...
export const KORISNIK_PALETA_DASHBOARD_STATUS: Record<
  'u_obradi' | 'potvrdeno' | 'u_toku' | 'zavrseno' | 'zatvoreno' | 'otkazano' | 'odbijeno',
...
> = {
  ...
  u_toku:    { pozadina: 'rgb(var(--first-secondary-rgb) / 0.14)',   boja: 'var(--first-secondary)' },
  zavrseno:  { pozadina: 'rgba(22,101,52,0.10)',                     boja: '#166534'                },
  zatvoreno: { pozadina: 'rgb(var(--first-quinary-rgb) / 0.3)',      boja: 'var(--first-nonary)'    },
  otkazano:  { pozadina: 'rgb(var(--first-quinary-rgb) / 0.3)',      boja: 'var(--first-nonary)'    },
  odbijeno:  { pozadina: 'rgb(var(--first-senary-rgb) / 0.2)',       boja: 'var(--first-senary)'    },
};
...
export function statusOznaka(s: string): string {
  switch (s) {
    case 'pending_review':
    case 'na_cekanju':   return 'Novi zahtjev';
    case 'in_review':    return 'U obradi';
    case 'potvrdeno':    return 'Čeka dodjelu';
    ...
    case 'zatvoreno':    return 'Zatvoreno';
