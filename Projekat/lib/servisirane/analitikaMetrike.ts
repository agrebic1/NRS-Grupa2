...
export function prosjek(vrijednosti: number[]): number | null {
  ...
  return Math.round(
    vrijednosti.reduce((s, v) => s + v, 0) / vrijednosti.length,
  );
}
...
export function jeSlaIspunjen(
  ...
  const slaMs =
    SLA_ROKOVI_SATI[final_priority as keyof typeof SLA_ROKOVI_SATI] * 3_600_000;
  const trajMs =
...
export function opterecenjePoServiseru(
  ...
  for (const z of sviZahtjevi) {
    if (z.serviser_dodijeljen_id && AKTIVNI_STATUSI.has(z.status)) {
      aktivni.set(
        ...
        (aktivni.get(z.serviser_dodijeljen_id) ?? 0) + 1,
      );
    }
  }
  for (const z of zavrseniZahtjevi) {
    if (z.serviser_dodijeljen_id) {
      zavrseni.set(
        z.serviser_dodijeljen_id,
        (zavrseni.get(z.serviser_dodijeljen_id) ?? 0) + 1,
      );
    }
  }
...
export function sastaviMetrike(ulaz: AnalitikaUlaz): AnalitikaMetrike {
  return {
    ...
    opterecenje_servisera: opterecenjePoServiseru(
      ulaz.sviZahtjevi,
      ulaz.zavrseniZahtjevi,
      ulaz.imenaServisera,
    ),
