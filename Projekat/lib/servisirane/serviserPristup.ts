...
const TERMINALNI_STATUSI = new Set([
  'zavrseno',
  'zatvoreno',
  'otkazano',
  'odbijeno',
...
export async function provjeriServiserPristupDetalju(
  ...
  if (TERMINALNI_STATUSI_ARHIVA.has(status)) {
    const imaEvidenciju = await serviserImaEvidenciju(
      supabase,
      zahtjevId,
      serviserId,
    );
...
export async function assertServiserVlasnistvo(
  ...
  | { ok: true; status: string; is_premium: boolean }
  | { ok: false; greska: string }
> {
  const pristup = await provjeriServiserPristupDetalju(
    supabase,
    zahtjevId,
    servizerId,
  );
