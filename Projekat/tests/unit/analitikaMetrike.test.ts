...
function zahtjev(
  over: Partial<AnalitikaZahtjevRed> & { id: number },
): AnalitikaZahtjevRed {
  return {
    status: 'na_cekanju',
    final_priority: null,
...
describe('jeSlaIspunjen', () => {
  test('null kad nema prioriteta', () => {
    expect(
      jeSlaIspunjen(null, '2026-05-01T00:00:00Z', '2026-05-01T01:00:00Z'),
    ).toBeNull();
  });
  test('true kad je unutar roka (SREDNJE = 24h)', () => {
    expect(
      jeSlaIspunjen('SREDNJE', '2026-05-01T00:00:00Z', '2026-05-01T10:00:00Z'),
    ).toBe(true);
  });
  test('false kad je prekoračen rok (VISOKO = 8h)', () => {
    expect(
      jeSlaIspunjen('VISOKO', '2026-05-01T00:00:00Z', '2026-05-02T00:00:00Z'),
    ).toBe(false);
  });
});
...
describe('opterecenjePoServiseru', () => {
  test('broji aktivne i završene po serviseru', () => {
    ...
    const r = opterecenjePoServiseru(svi, zavrseni, {
      s1: 'Ana A',
      s2: 'Boris B',
    });
