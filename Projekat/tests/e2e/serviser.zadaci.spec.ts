...
test.describe('Serviser - pregled zadataka i RBAC', () => {
  ...
  test('serviser može otvoriti dashboard i dobiti listu intervencija', async ({
    page,
  }) => {
  ...
  test('serviser može otvoriti stranicu detalja zadatka (ako postoji dodijeljena)', async ({
    page,
  }) => {
    await prijaviSe(page, serviser as RoleCreds);
    ...
    if (prvaIntervencija?.id) {
      const detalj = await page.goto(
        `/serviser/intervencije/${prvaIntervencija.id}`,
      );
      expect(detalj?.ok()).toBeTruthy();
    }
  });

  test('serviser ne može pristupiti dispečerskim API rutama → 403', async ({
    page,
  }) => {
    await prijaviSe(page, serviser as RoleCreds);
  ...
  test('dispečer ne može koristiti serviser PATCH endpoint → 403', async ({
    page,
  }) => {
    test.skip(
      !dispecer,
