...
export function RutaMapa({
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver(() => {
      ...
      setW(newW);
      setZoom(
        calculateFitZoom(baznaLat, baznaLng, destLat, destLng, newW, visina),
      );
    ...
    setW(initW);
    setZoom(
      calculateFitZoom(baznaLat, baznaLng, destLat, destLng, initW, visina),
    );
  ...
  return (
    <div
      ...
      onWheel={(e) => {
        e.preventDefault();
        setZoom((prev) =>
          e.deltaY < 0 ? Math.min(prev + 1, 18) : Math.max(prev - 1, 3),
        );
      }}
      role="img"
    ...
    >
      ...
      >
        <MapPin
          style={{ width: 32, height: 32, color: 'var(--first-primary)' }}
        />
      </div>
      ...
      >
        {[
          {
            label: '+',
            action: () => setZoom((z) => Math.min(z + 1, 18)),
            aria: 'Približi mapu',
          },
          {
            label: '−',
            action: () => setZoom((z) => Math.max(z - 1, 3)),
            aria: 'Udalji mapu',
          },
