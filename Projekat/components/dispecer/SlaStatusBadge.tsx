...
import { getSlaStatus, formatirајPreostaloVrijeme } from '@/lib/servisirane/slaPravila';
...
const KONFIGURACIJA: Record<SlaStatus, {
  boja:    string;
  rgb:     string;
  tekst:   string;
  Ikona:   typeof Clock;
...
export function SlaStatusBadge({ createdAt, prioritet, status, prikaziVrijeme = false }: SlaStatusBadgeProps) {
  const slaStatus = getSlaStatus(createdAt, prioritet, status);
  if (!slaStatus) return null;

  const cfg        = KONFIGURACIJA[slaStatus];
  const Ikona      = cfg.Ikona;
  const vrijemeTxt = prikaziVrijeme ? formatirајPreostaloVrijeme(createdAt, prioritet) : null;
  ...
  return (
    ...
    >
      ...
      {cfg.tekst}{vrijemeTxt ? ` (${vrijemeTxt})` : ''}
    </span>
  );
}
