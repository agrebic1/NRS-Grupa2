...
import { Star, MessageSquare, Check, Loader2, AlertTriangle } from 'lucide-react';
...
function Zvjezdice({
  ...
  return (
    <div className="flex items-center gap-1" role={interaktivno ? 'radiogroup' : undefined} aria-label="Ocjena">
      {[1, 2, 3, 4, 5].map((n) => {
        const popunjeno = interaktivno ? n <= (hover || vrijednost) : n <= vrijednost;
        return (
          <button
            key={n}
            type="button"
            ...
            style={{
              transform: interaktivno && hover === n ? 'scale(1.2)' : 'scale(1)',
            }}
          >
            <Star
              className="h-7 w-7"
              ...
              style={{ color: popunjeno ? 'var(--first-senary)' : 'rgb(var(--first-quaternary-rgb)/0.6)' }}
            />
          </button>
        );
      })}
...
export function OcjenaIntervencije({ zahtjevId }: OcjenaIntervencijeProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/service-requests/${zahtjevId}/ocjena`, { cache: 'no-store' });
  ...
  async function posaljiOcjenu() {
    if (odabranaOc === 0) { setGreska('Odaberite ocjenu.'); return; }
    setJeSlanje(true);
    setGreska(null);
    try {
      const r = await fetch(`/api/service-requests/${zahtjevId}/ocjena`, {
        ...
        body:    JSON.stringify({ ocjena: odabranaOc, komentar: komentar.trim() || null }),
      });
      const d = await r.json();
  ...
  if (ucitava) {
    return (
      <div className="flex items-center gap-2 py-2">
        <Loader2 className="h-4 w-4 animate-spin" style={{ color: 'var(--first-nonary)' }} />
        <span className="text-sm" style={{ color: 'var(--first-nonary)' }}>Učitavanje ocjene...</span>
      </div>
    );
  }
  ...
  if (ocjena) {
    return (
      <div className="flex flex-col gap-3">
        ...
        {ocjena.komentar && (
          <div className="flex gap-2 rounded-xl border-l-4 px-4 py-3"
      ...
      {uspjeh ? (
        <div className="flex items-center gap-2 rounded-xl px-4 py-3"
          ...
          <p className="text-sm font-medium" style={{ color: '#16A34A' }}>
            Hvala na ocjeni! Vaša povratna informacija pomaže nam unaprijediti uslugu.
          </p>
        </div>
      ) : (
        <>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-3">
              <Zvjezdice
                ...
                hover={hoverOc}
                onHover={setHoverOc}
                onClick={(n) => { setOdabranaOc(n); setGreska(null); }}
              />
              {(hoverOc > 0 || odabranaOc > 0) && (
                <span className="text-sm font-semibold transition-all" style={{ color: 'var(--first-senary)' }}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            />
            <p className="text-right text-xs" style={{ color: 'var(--first-nonary)' }}>
