...
export async function geocodeAdresa(adresa: string): Promise<NominatimResult[]> {
  try {
    const r = await fetch(
...
export function AddressAutocomplete({
  ...
  return (
    ...
    >
      <label htmlFor={inputId} className="text-sm font-medium" style={{ color: 'var(--first-octonary)' }}>
        {label}
      </label>
...
export interface OdabirLokacijeProps {
  ...
  onChange:   (c: { latitude: number | null; longitude: number | null; adresa?: string }) => void;
  label?:     string;
  helperText?: string;
...
export function OdabirLokacije({
  ...
  useEffect(() => {
    if (latitude != null && longitude != null) {
      ...
      setZoom(15);
      reverseGeocode(latitude, longitude).then((a) => { if (a) setAdresa(a); });
    }
  ...
  async function handlePin(coords: { lat: number; lon: number }) {
    ...
    if (a) setAdresa(a);
    onChange({ latitude: coords.lat, longitude: coords.lon, adresa: a ?? undefined });
  }

  function locirajMe() {
    if (typeof window !== 'undefined' && !window.isSecureContext) {
      setGreska('Lokacija u pregledniku radi samo preko HTTPS (ili na localhostu).');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setIsLocating(false);
        setGreska('Nije moguće dohvatiti trenutnu lokaciju. Provjerite dozvole.');
      },
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className={`grid grid-cols-1 gap-3 ${prikaziGps ? 'sm:grid-cols-2' : ''}`}>
        {prikaziGps && (
          <button
            type="button"
            onClick={locirajMe}
      ...
      {mapVisible && (
        <div
          ...
          style={{ borderColor: 'var(--border-info)', backgroundColor: 'rgb(var(--first-septenary-rgb) / 0.08)' }}
        >
          <div className="flex items-start justify-between gap-3">
            ...
            {pin && (
              <button
                ...
                style={{ borderColor: 'rgba(220,38,38,0.35)', color: '#DC2626', backgroundColor: '#fff' }}
              >
