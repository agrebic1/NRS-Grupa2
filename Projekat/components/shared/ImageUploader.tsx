...
export function ImageUploader({
  ...
  const dodajFajlove = useCallback(
    async (fajlovi: File[]) => {
  ...
  async function uploadSliku(slika: SlikaPreview) {
    ...
    try {
      ...
      setSlike((prev) =>
        prev.map((s) =>
          s.id === slika.id ? { ...s, status: 'ok', url, dbId } : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === slika.id
            ? {
                ...s,
                status: 'greska',
                greska: e instanceof Error ? e.message : 'Greška',
              }
            : s,
        ),
      );
  ...
  async function ukloni(id: string) {
    ...
    if (slika.status === 'ok' && slika.dbId && mozeBrisati) {
      try {
        const r = await fetch(`/api/slike?id=${slika.dbId}`, {
          method: 'DELETE',
        });
        const d = await r.json();
          ...
          {slike.map((s) => (
            ...
            >
              ...
              <img
                ...
                onClick={() =>
                  s.status === 'ok' && setLightbox(s.url ?? s.dataUrl)
                }
              />
